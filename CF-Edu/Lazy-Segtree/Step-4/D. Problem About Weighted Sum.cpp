...
LazySeg<pair<int, int>, pair<int, int>, 1 << 18> s({0, 0}, {0, 0}, [](pair<int, int> a, pair<int, int> b) {
	return make_pair(a.first + b.first, a.second + b.second);
}, [](int ind, int L, int R, vector<pair<int, int>> &seg, vector<pair<int, int>> &lazy) {
	seg[ind].first += ((R-L+1) * (2 * lazy[ind].first + (R-L)*lazy[ind].second)) / 2; // dependent on operation
	seg[ind].second += (R-L+1) * lazy[ind].second;
	if (L != R) {
		int M = (L + R) / 2;
...
int32_t main() {
	...
	while (q--) {
		...
		if (o == 1) {
			...
			int l, r;
			input(l, r);
			pair<int, int> p = s.query(l - 1, r - 1);
			print(p.first - (l - 1) * p.second);
		}
	}
}
