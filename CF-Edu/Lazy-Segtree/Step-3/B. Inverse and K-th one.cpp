...
template<class T, class U, int SZ> struct LazySeg { 
	...
	int search(int k, int ind = 1, int L = 0, int R = SZ - 1) {
		push(ind, L, R, seg, lazy);
		if (L == R) {
			return L;
		}
		int M = (L + R) / 2;
		push(2 * ind, L, M, seg, lazy);
		if (seg[2*ind] >= k) {
			return search(k, 2 * ind, L, M);
		}
		return search(k - seg[2 * ind], 2 * ind + 1, M + 1, R);
	}
};
...
LazySeg<int, int, 1 << 18> s(0, 0, [](int a, int b) {
	...
	seg[ind] = (R-L+1)-seg[ind]; // dependent on operation
	if (L != R) {lazy[2*ind] ^= lazy[ind]; lazy[2*ind+1] ^= lazy[ind];}
	lazy[ind] = 0; /// prop to children
});
