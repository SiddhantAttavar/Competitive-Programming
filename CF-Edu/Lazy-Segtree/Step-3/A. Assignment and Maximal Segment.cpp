...
LazySeg<array<int, 4>, int, 1 << 18> s({0, 0, 0, 0}, 1e18, [](array<int, 4> a, array<int, 4> b) {
	return array<int, 4>{a[0] + b[0], max(a[1], a[0] + b[1]), max(a[2] + b[0], b[2]), max({a[3], b[3], a[2] + b[1]})};
}, [](int ind, int L, int R, vector<array<int, 4>> &seg, vector<int> &lazy) {
	if (lazy[ind] == 1e18) return;
	seg[ind] = {(R-L+1)*lazy[ind], max(0ll, (R-L+1)*lazy[ind]), max(0ll, (R-L+1)*lazy[ind]), max(0ll, (R-L+1)*lazy[ind])}; // dependent on operation
	if (L != R) {lazy[2*ind] = lazy[ind]; lazy[2*ind+1] = lazy[ind];}
	lazy[ind] = 1e18; /// prop to children
});
